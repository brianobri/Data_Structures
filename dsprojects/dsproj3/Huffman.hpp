...
{
    if (node->value == NULL){  // not a node
        return;
    }
    if (node->left){  // if there is a left add 0 to the code call  left child
        create_codes(node->left, code + '0');
    }
    if (node->right){ // if there is a right add 1 to the code call right child
        create_codes(node->right, code + '1');
    }
    if(!node->left && !node->right){  // no left or right child, add code to the pair
        codes.insert(std::make_pair(node->value, code)).second;
    }
...
void Huffman::serialize_tree(HNode* node, std::string& s) {
    if (node){  // interior or leaf node
        s = s + node->value;
        serialize_tree(node->left, s);
        serialize_tree(node->right, s);
    }
    else if (!node){
        s = s + "/"; // no node below the leaf add slashes to serialize
    }
...
void Huffman::encode_string(const std::string& input, std::string& encoded_string) {
    // thought about using a foreach loop
    for(int c = 0; c < input.length(); c++){  // finds code for each letter in the input, adds it to a string
        encoded_string = encoded_string + codes.find(input[c])->second;
    }
